		...
		function($translate, LOCALES, $rootScope, tmhDynamicLocale) {
			...
			var setLocale = function(locale) {
				if (!checkLocaleIsValid(locale)) {
					console
						.error('Locale name "' + locale +
							'" is invalid');
					return;
				}
				currentLocale = locale; // updating current locale

				// asking angular-translate to load and apply proper
				// translations
			...
			$rootScope.$on('$translateChangeSuccess',
				function(event, data) {
					...
					// asking angular-dynamic-locale to load and apply
					// proper AngularJS $locale setting
